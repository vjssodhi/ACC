...
class UserORM {
	...
	public function updateUser($id, $data) {
		...
		/* @var $user User */
		$oldEmpCode = $user->getEmployeeCode ();
		...
		if (! empty ( $data ['loginId'] ) && ($data ['loginId'] !== $oldLoginId)) {
			$userG = $this->fetchAll ( array (
					'loginId' => $data ['loginId'] 
			) ) [0];
			if (! empty ( $userG )) {
				$errors ['loginId'] [] = sprintf ( 'Login id: %s is already registered', $data ['loginId'] );
				$problem = true;
			} else {
		...
		if (! empty ( $data ['personalEmailId'] ) && ($data ['personalEmailId'] !== $oldPersonalEmailId)) {
			$userXT = $this->fetchAll ( array (
					'personalEmailId' => $data ['personalEmailId'] 
			) ) [0];
			if (! empty ( $userXT )) {
				$errors ['personalEmailId'] [] = sprintf ( 'Email id: %s is already registered', $data ['personalEmailId'] );
				$problem = true;
			} else {
				$user->setPersonalEmailId ( $data ['personalEmailId'] );
			}
		}
		if (! empty ( $data ['employeeCode'] ) && ($data ['employeeCode'] !== $oldEmpCode)) {
			$userEMXT = $this->fetchAll ( array (
					'employeeCode' => $data ['employeeCode'] 
			) ) [0];
			if (! empty ( $userEMXT )) {
				$errors ['employeeCode'] [] = sprintf ( 'Employee Code: %s is already registered', $data ['employeeCode'] );
				$problem = true;
			} else {
		...
		if (! empty ( $data ['reportingPath'] )) {
			$user->setReportingPath ( $data ['reportingPath'] );
		}
		...
		try {
			$userId = $user->getId ();
			if (! empty ( $seniorId )) {
				$rTree = new RestreeORM ();
				$rTree->setOrmEntityMgr ( $om );
				$rTree->setConnection ( $connection );
				$newPath = $rTree->changeNodeParent ( $userId, static::USER_ENTITY, $seniorId, static::USER_ENTITY );
				$user->setSelfPath ( $newPath );
				$user->setReportingPath ( $data ['reportingPath'] );
				$user->setSelfPath ( $newPath );
			}
			$om->flush ( $user );
	...
	public function registeruser($data) {
		...
		if (empty ( $data ['employeeCode'] )) {
			if ($requiredFields ['employeeCode']) {
		...
		try {
			...
			$rTree->setConnection ( $connection );
			if (empty ( $seniorId )) {
				$dna = $rTree->save ( static::USER_ENTITY, $userId );
			} else {
				$dna = $rTree->save ( static::USER_ENTITY, $userId, static::USER_ENTITY, $seniorId );
			}
			$user->setSelfPath ( $dna );
			$om->flush ( $user );
	...
	public function fetchAll($searchParams, $getQ = false, $returnObject = false, $returnedInfo = null) {
		if (! empty ( $searchParams )) {
			$searchParams = NumberPlay::cleaner ( $searchParams, array (
					'password',
					'passwordConfirm' 
			) );
		...
		if (isset ( $searchParams ['employeeCode'] )) {
			$params [':empC'] = $searchParams ['employeeCode'];
			$qbs->andWhere ( 'al.employeeCode = :empC' );
		}
		...
		if (isset ( $searchParams ['selfPath'] )) {
			$params [':spth'] = $searchParams ['selfPath'];
			$qbs->andWhere ( 'al.selfPath = :spth' );
		}
	...
	public function processPassword($loginId, $userPasswordFromInput, $rehashIfNeeded = false) {
		...
		$passwordMacthes = password_verify ( $userPasswordFromInput, $currentHashFromPersistentStorage );
		if (! $passwordMacthes) {
			return false;
		}
		...
		if ($passwordMacthes && $rehashIfNeeded === true) {
			...
			$algo = $passInfo ['algo'];
			$needRehash = password_needs_rehash ( $currentHashFromPersistentStorage, $algo, $new_password_options );
			if ($needRehash) {
